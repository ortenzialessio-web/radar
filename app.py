...
for name, tickers in PURE_PORTFOLIOS.items():
    ...
    if rows:
        ...
        portfolio_dataframes[name] = df_temp
        summary_rows.append({
            "SCENARIO MACRO": name,
            "1G %": df_temp["1G %"].mean(),
            "1M %": df_temp["1M %"].mean(),
...
# 5. DICHIARAZIONE ESPLICITA DELLE SCHEDE (Risolve il problema dei Tab invisibili)
tab_names = ["📊 COMPARA", "GOLDILOCKS", "RECESSION", "STAGFLATION", "REFLATION", "HARD ASSETS", "SOCIETÀ PRODUTTRICI", "DEBASEMENT (CON BTC)", "PORTAFOGLIO KT"]
t_compara, t_goldi, t_recess, t_stag, t_refl, t_hard, t_prod, t_debase, t_kt = st.tabs(tab_names)
...
with t_compara:
    ...
    if not df_summary.empty:
        ...
        st.error("Nessun dato aggregato disponibile.")

# --- FUNZIONE INTERNA PER DISEGNARE I DETTAGLI (Azzera le ripetizioni nel codice) ---
def render_portfolio_details(name):
