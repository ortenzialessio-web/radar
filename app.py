...
# Intestazione minimalista e raffinata
...
def fetch_financial_data(all_tickers):
    ...
    for ticker in all_tickers:
        try:
            ...
            price_1m = hist['Close'].iloc[-22] if len(hist) >= 22 else hist['Close'].iloc
            
            hist_ytd = hist[hist.index.year == today.year]
            price_ytd = hist_ytd['Close'].iloc if not hist_ytd.empty else current_price
            price_1an = hist['Close'].iloc[-252] if len(hist) >= 252 else hist['Close'].iloc
...
# 5. Rendering dei Tab con Stile Custom applicato
tabs = st.tabs(list(PORTFOLIOS.keys()))

# --- TAB 0: COMPARA SCENARI (Visual Leaderboard) ---
with tabs[0]:
    ...
    if not df_summary.empty:
        # Grafico a barre con layout scuro nativo
        st.bar_chart(df_summary.set_index("SCENARIO MACRO")[["YTD %"]], use_container_width=True)
        
        st.write("")
        # Tabella riepilogativa formattata in modo chirurgico
        st.dataframe(
...
for idx, (name, tickers) in enumerate(PORTFOLIOS.items()):
    ...
    with tabs[idx]:
        ...
        if rows:
            ...
            # Griglia di metriche premium (Stile Widget Smartphone)
            ...
            # Tabella del portafoglio singolo con colori accesi ad alto contrasto per Dark Mode
            st.dataframe(
                df.style.format({
