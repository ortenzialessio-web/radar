...
# 3. Definizione degli 8 Portafogli (Rimossi: Cash Preferenziale e Debasement senza BTC)
PORTFOLIOS = {
    "📊 COMPARA": [], 
...
def fetch_financial_data(all_tickers):
    ...
    for ticker in all_tickers:
        try:
            ...
            # Estrazione del nome completo dell'ETF/Asset
            full_name = t_obj.info.get('longName', t_obj.info.get('shortName', 'N/A'))
            
            data_dict[ticker.upper()] = {
                "Ticker": ticker.upper(),
                "Nome Asset": full_name,
...
with tabs:
    ...
    if not df_summary.empty:
        # GRAFICO ORIZZONTALE (Utilizzo di st.bar_chart con dati invertiti per forzare l'asse orizzontale)
        # Per mostrare le barre in orizzontale in Streamlit, usiamo st.bar_chart con layout nativo orizzontale (horizontal=True)
